...
export class ContractComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        // if (this.createPagePermission) {
        if ((!id && this.addPermission) || (id && this.updatePermission)) {
    ...
    onSave() {
        ...
        //     ContractExpiryDate: this.contract.contractExpiryDate,
        //     ExternalPartyId: this.contract.externalPartyId,
        //     ContractDocument: this.contract.contractDocs
        // };

        // var test = false;
        // if (test) {
        // if (this.createPagePermission) {
        // if ((this.addPermission && !this.contract.id) || (this.updatePermission && this.contract.id)) {
        if (this.addPermission) {
            ...
            if (this.contractFormGroup.valid) {
                ...
                // console.log('contarct', obj);
                console.log('contarct', this.contract);
                // this._contractService.saveContract(obj).takeUntil(this.ngUnsubscribe).subscribe(
                this._contractService.saveContract(this.contract).takeUntil(this.ngUnsubscribe).subscribe(
                    (res) => {
                        console.log(res);
                        this.isSubmitted = false;
                        this._uiService.hideSpinner();
                        msg.msg = res.json() ? res.json().message : 'Contract added successfully.';
                        msg.msgType = MessageTypes.Information;
                        msg.autoCloseAfter = 400;
                        this._uiService.showToast(msg, 'info');
    ...
    onUpdate() {
        ...
        if (this.updatePermission) {
            ...
            if (this.contractFormGroup.valid) {
                ...
                this._contractService.updateContract(this.contract).takeUntil(this.ngUnsubscribe).subscribe(
                    (res) => {
                        console.log(res);
                        this.isSubmitted = false;
                        this._uiService.hideSpinner();
